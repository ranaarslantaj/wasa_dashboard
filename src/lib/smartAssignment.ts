...
/**
 * Rank active employees against a complaint using specialization, geographic overlap, and workload.
 * Pure function — no Firestore calls.
 */
export const rankEmployees = (
  employees: WasaEmployee[],
  complaint: Pick<Complaint, 'complaintType' | 'province' | 'division' | 'district' | 'tehsil'>,
): RankedEmployee[] => {
  return employees
    .filter((e) => e.active)
    .map((e) => {
      ...
      if (e.specialization?.includes(complaint.complaintType)) {
        score += 100;
        reason.push('specialization match');
      }

      if (e.tehsil === complaint.tehsil) {
        score += 50;
        reason.push('same tehsil');
      } else if (e.district === complaint.district) {
        score += 30;
        reason.push('same district');
      } else if (e.division === complaint.division) {
        score += 10;
        reason.push('same division');
      }
    ...
    .sort(
      (a, b) =>
        b.score - a.score || (a.currentAssignments ?? 0) - (b.currentAssignments ?? 0),
    );
};
